...
then
	if [ $ARCH == "i686" ];
	then
		sudo apt-get install qt4-dev-tools:i386 libqjson-dev:i386 libqwt-dev:i386 libc6-dev-i386 gcc-multilib g++-multilib python3-dev libpython3-dev &&
	...
	then
		sudo apt-get install qt4-dev-tools libqjson-dev libqwt-dev python3-dev libpython3-dev &&
		git clone git://code.qt.io/qt/qtserialport.git &&
		cd qtserialport &&
		git checkout qt4-dev
...
then
	sudo apt-get install autopoint intltool gperf cmake scons liblzma-dev python3-dev libpython3-dev &&
